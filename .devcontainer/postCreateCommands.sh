...
docker exec -it wordpress-gdata-antivirus-app-1 bash -c "wp core install --url=http://localhost:8080 --title=testsite --admin_user=admin --admin_email=vaas@gda
ta.de --admin_password=admin"